...
/* legend */
      ...
      */
      ...
      /* Sets progress count shown ONLY for "In Progress" (yellow) cards */
      .band-count{
        font-size: 12px;
        font-weight: 800;
        letter-spacing: .06em;
      ...
      */
      ...
      #results .band-card .band-count{ display:inline !important; }
...
async function downloadZipFromServer(items, suggestedName){
  ...
  function showLetter(region, letter) {
    ...
    bandsArr.forEach((bandObj) => {
      ...
      // Show ( archived / total ) ONLY for "In Progress" (yellow) list cards
      if (_setsState === "setsPartial") {
        const archived = Number(bandObj?.sets_archive);
        const total = Number(bandObj?.total_sets);
        if (Number.isFinite(archived) && Number.isFinite(total) && total > 0) {
          const cnt = document.createElement("span");
          cnt.className = "band-count";
          cnt.textContent = `( ${archived} / ${total} )`;
