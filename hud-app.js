...
const TEMP_ALWAYS_SHOW_TESTING = false;
  ...
  (function(){
    ...
    try { window.__vmIsAdminUnlocked = isAdminUnlocked; } catch (_) {}
...
function pulseFrame(){
  ...
  function setActiveTopNav(route){
    ...
    const VALID_ROUTE_KEYS = new Set(['home','music','wrestling','testing','calendar','about','pricing','contact','admin']);
  ...
  async function canAccessProtectedRoute(route){
    ...
    try {
      const check = (typeof window.__vmVerifyAdminAccess === 'function')
        ...
        : !!(typeof window.__vmIsAdminUnlocked === 'function' && window.__vmIsAdminUnlocked());
      if (check) return true;
    } catch (_) {}
    try {
      if (typeof window.__vmOpenAdminModal === 'function') window.__vmOpenAdminModal();
    } catch (_) {}
        ...
        m.innerHTML = `
          <div id="vmTestingPanelRoot" style="width:100%; max-width:none; margin:0; padding:0; min-height:100dvh; height:100dvh;">
            <div style="border:0; border-radius:0; overflow:hidden; background:transparent; min-height:100dvh; height:100dvh; box-shadow:none;">
              <iframe id="vmTestingRouteFrame" src="/archive-explorer-mockup.html?v=20260420-testing-music-route-r1&shellRoute=${encodeURIComponent(shellRoute)}" title="Archive Explorer Mockup" style="display:block; width:100%; min-width:0; height:100%; min-height:100dvh; border:0; background:transparent;"></iframe>
            </div>
          </div>
        `;
...
async function transitionTo(route){
  ...
  document.body.classList.remove('route-home','route-music','route-wrestling','route-testing','route-calendar','route-about','route-pricing','route-contact');
